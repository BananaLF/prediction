...
理论上可先 `SPLIT` 一单位抵押品得到 YES 和 NO，再分别卖出。只有“拆分能力、转换成本、卖盘对手深度、费用和结算语义”均被完整建模并有证据时，才能计算：

```text
最低卖出所得 - 拆分投入 - 费用 - 转换成本 - 安全缓冲
```

当前扫描器没有把该路径实现为可执行分类，因此只应视为研究原理，不能据此操作。
...
| 信息迟到、缺失或陈旧 | 全部 | 使用已经失效的价格 | 交易所时间、接收时间、单调时钟、最大书龄、腿间偏斜和处理延迟均为硬门 |
...
| 部分成交与退出 | 全部 | 产生未对冲损失 | 计算单腿最坏失败损失、未对冲名义金额及退出深度；超过门槛即拒绝 |
...
| 通知失败或重复 | 可执行快照 | 漏提醒或重复提醒 | SQLite 是事实来源；通知采用租约 outbox 的至少一次语义；保存 claim/attempt/event 审计 |

## 简化架构

```mermaid
flowchart LR
    A["公开 WS<br/>仅发现"] --> B["公开 REST<br/>两次确认"]
    B --> C["深度模拟<br/>收益与风险"]
    C --> D["SQLite<br/>完整证据"]
